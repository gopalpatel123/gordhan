...
class BillsController extends AppController
{
    ...
    public function index()
    {
        ...
        $q_path=$_SERVER["QUERY_STRING"];

        $where=[];
         $where['Bills.is_deleted']='no';

        $bill_no=$this->request->query('bill-no');
        if(!empty($bill_no)){
...
/* 
        ...
        $amount_from=$this->request->query('amount-from');
        if(!empty($amount_from)){
            $where['Bills.grand_total >=']=$amount_from;
        }

        $amount_to=$this->request->query('amount-to');
        if(!empty($amount_to)){
            $where['Bills.grand_total <=']=$amount_to;
        }

        $customer_name=$this->request->query('customer-name');
        if(!empty($customer_name)){
            $where['Customers.name LIKE']='%'.$customer_name.'%';
        }

        $mobile_no=$this->request->query('mobile-no');
        if(!empty($mobile_no)){
            $where['Customers.mobile_no LIKE']='%'.$mobile_no.'%';
        }

        $customer_code=$this->request->query('customer-code');
        ...
        $bills = $this->paginate(
            $this->Bills->find()->where($where)->order(['voucher_no'=>'DESC'])
        );
        ...
        $this->set(compact('bills', 'bill_no', 'from_date', 'to_date', 'amount_from', 'amount_to', 'customer_name', 'mobile_no', 'customer_code', 'query', 'q_path'));
    }
    ...
    public function delete($id = null)
    {
        ...
        $q_path=$this->request->data(['q_path']);
        ...
        return $this->redirect(['action' => 'index?'.$q_path]);
    }
    ...
    public function categoryWiseSales(){
		...
		->select([
            ...
            'Category_name' => 'ItemCategories.name',
			 'category_id' => 'ItemSubCategories.item_category_id'
        ])
        ->where(['Bills.transaction_date' => date('Y-m-d'), 'Bills.is_deleted'=>'no'])->toArray();
		
		$Rows1=$this->Bills->BillRows->find();
		$Rows1->innerJoinWith('Items.ItemSubCategories.ItemCategories')
        ->innerJoinWith('Bills')
        ->group(['ItemSubCategories.id'])
		->where(['ItemSubCategories.item_category_id' =>9])
        ->select([
            'TotalSale' => $Rows1->func()->sum('BillRows.net_amount'),
            'Totalquantity' => $Rows1->func()->sum('BillRows.quantity'),
            'Category_name' => 'ItemSubCategories.name'
           
        ])
        ->where(['Bills.transaction_date' => date('Y-m-d'), 'Bills.is_deleted'=>'no'])->toArray(); 
		//pr($Rows1->toArray()); exit;
		
        $this->set(compact('Rows','Rows1'));
    }
