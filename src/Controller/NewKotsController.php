 ...
 */
class NewkotsController extends AppController
{
    ...
    {
        $this->viewBuilder()->layout('');
        $Kots=$this->Kots->find()->where(['Kots.id'=>$kot_id])->contain(["Bills" => ['Employees', 'Customers'],'KotRows'=> function($q){
            return $q->where(['KotRows.is_deleted' => 0])->contain(['Items'=>['Taxes']]);
        } ])->first();
    ...
    public function add()
    {
		...
		$last_voucher_no=$this->Kots->find()->select(['voucher_no'])
                        ->where(['Kots.financial_year_id'=>2,'Kots.table_id' => $table_id, 'Kots.order_type' => $order_type, 'Kots.bill_pending' => 'yes'])
                        ->order(['id' => 'DESC'])->first();
		if($last_voucher_no){
