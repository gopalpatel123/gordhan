	...
	<table width="100%">
		...
		<?php foreach ($Rows as $Row) { 
			if($Row->category_id==9){
				$per1=round($Row->TotalSale*100/$GrandTotal, 2);
			}else{
			$per=round($Row->TotalSale*100/$GrandTotal, 2);
			?>
			<tr>
				<td width="40%"><h6 style="font-size: 15px;font-weight: 200;color: #7E8082;">
			...
			<?php } } ?>
		<?php foreach ($Rows1 as $Row) {  //echo $per1; echo $Row->TotalSale; exit;
			$per=$per1;
			?>
			<tr>
				<td width="40%"><h6 style="font-size: 15px;font-weight: 200;color: #7E8082;">
					<?php echo $Row->Category_name.' ('. $Row->Totalquantity.')'; ?>
				</h6></td>
				<td width="15%"><h6 style="font-size: 15px;font-weight: 200;color: #656363;font-weight: bold;">₹ <?php echo $Row->TotalSale; ?></h6></td>
				<td width="5%"></td>
				<td width="35%">
