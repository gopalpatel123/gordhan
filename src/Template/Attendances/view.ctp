...
<style>
	.groupHeading{
		font-size: 12px;background-color: #2d4161;color:  #FFF;padding: 2px 4px;
	}
</style>
        ...
        <div class="portlet box blue-hoki">
            <div class="portlet-title hide_at_print">
                <table width="100%" style=" margin-top: 5px; margin-bottom: 5px; ">
                    <tr>
                        <td width="30%">
                            <div class="caption"style="padding:13px 0 10px 32px; color: red;">
                                Attendance Sheet
                            </div>
                        </td>
                        <td valign="button">
                    ...
                    <div class="ContenedorTabla">
                       <!--  <table class="table table-bordered table-stripped fht-table" id="pruebatabla4"> -->
                        <table class="table table-bordered table-stripped fht-table" id="pruebatabla4">
                            <thead>
                                <tr>
                                   <th style="background-color: #c8e8ff; font-size13px; font-weight:bold; line-height: 34px; letter-spacing:1px;">&nbsp;EMPLOYEE</th>
                                   <?php $tmp=0;
                                    $firstDate = $month1[1].'-'.$month1[0].'-1';
                                    $lastDate = date("Y-m-t", strtotime($firstDate));
                                    while (strtotime($firstDate) <= strtotime($lastDate)) { $tmp++;
                                        echo '<th style="white-space: nowrap; text-align:center; background-color: #c8e8ff; font-size:12px; line-height:34px; letter-spacing:1px; font-weight:bold;" >'.date('d-m-Y', strtotime($firstDate)).'</th>';
                            ...
                            <tbody>
                               <?php 
                                foreach ($Designations as $Designation) { //pr($Designation->name); exit; 
								?>
								...
								<tr>
									<td  style="padding: 0 !important;">
										...
										<div style="text-align:left;" class="groupHeading"> 
										<?= h($Designation->name) ?>
									</div>
									</td>
									<?php  ?>
									<td colspan="<?php echo $tmp; ?>" style="padding: 0 !important;">
										<div style="text-align:left;" class="groupHeading">.
										</div>
									</td>
								</tr>
								...
								?>
                                   <tr>
                                       <td style="">
                                            <div style="font-size: 12px;padding: 2px 4px;"><?= h($Employee->name) ?></div>
                                        ...
                                        while (strtotime($firstDate) <= strtotime($lastDate)) { 
                                            ...
                                            <td>
                                                <?php echo $this->Form->select(
                                                    ...
                                                    ['value' => @$data[$Employee->id][strtotime($firstDate)], 'empty'=>' ', 'class' => ' autoSave', 'date_string'=>strtotime($firstDate), 'employee_id'=>$Employee->id,
