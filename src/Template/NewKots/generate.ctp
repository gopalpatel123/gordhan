...
<?php
	...
	$(document).ready(function() {
		...
		$('.QtyCatcher').keypress(function(event){
		    ...
		    if(keycode == '13'){
				...
				if(item_id && Qty){
					...
					var c=$('#kotBox tbody tr.mainKotTr').length;
					c=c+1; 
					$('#kotBox').append('<tr row_no='+c+' gst_per='+gst_per+' class=mainKotTr><td style=text-align:center;>'+c+'</td><td item_id='+item_id+'>'+item_name+'</td><td style=text-align:center;><span class=\"minus\">-</span><span class=\"qty\"> '+Qty+' </span><span class=\"plus\">+</span></td><td style=\"text-align:center;padding: 5px;\">'+rate+'</td><td style=text-align:center;>'+rate+'</td><td style=\"padding:2px;\"><input type=text class=disPer /></td><td style=\"padding:2px;\"><input type=text class=disAmt /></td><td style=text-align:center;><i class=\"fa fa-ellipsis-h commentRow\" style=\"color: #BDBFC1; font-size: 18px; cursor: pointer;\"></i><textarea style=\"display:none;\" class=\"comment\"></textarea></td><td style=text-align:center;><i class=\"fa fa-trash-o removeRow\" style=\"color: #ff0000; font-size: 18px; cursor: pointer;\"></i></td></tr>');
					amountcals();
				}
				var Qty=$('.QtyCatcher').val(1);
				$('.ItemDropDown').select2('val',''); 
				$('.ItemDropDown').focus();
		    }
		    openDropdawn++;
		...
		$('.CreateBill').die().live('click',function(event){
            ...
            var customer_id=$('.customer_id_data').val();
			...
			url=url+'?myJSON='+myJSON+'&total='+total+'&roundOff='+roundOff+'&net='+net+'&c_name='+c_name+'&c_mobile_no='+c_mobile_no+'&c_address='+c_address+'&order_type='+order_type+'&employee_id='+employee_id+'&offer_id='+offer_id+'&oneComment='+oneComment+'&payment_type='+payment_type+'&c_address='+c_address+'&customer_id='+customer_id;
			url=encodeURI(url);
			$.ajax({
				url: url,
				dataType: 'json'
				...
				var wins = window.open(url, '_blank', 'shilpijain', 'modal=no');
				//wins.print();
				wins.close();
				...
				//location.reload();
				var url2='".$this->Url->build(['controller'=>'newKots','action'=>'generate/0/delivery'])."';
