<?php
/**
 * @var \App\View\AppView $this
 * @var \App\Model\Entity\MenuSubCategory[]|\Cake\Collection\CollectionInterface $menuSubCategories
 */
?>
<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        <li class="heading"><?= __('Actions') ?></li>
        <li><?= $this->Html->link(__('New Menu Sub Category'), ['action' => 'add']) ?></li>
        <li><?= $this->Html->link(__('List Menu Categories'), ['controller' => 'MenuCategories', 'action' => 'index']) ?></li>
        <li><?= $this->Html->link(__('New Menu Category'), ['controller' => 'MenuCategories', 'action' => 'add']) ?></li>
        <li><?= $this->Html->link(__('List Menu Items'), ['controller' => 'MenuItems', 'action' => 'index']) ?></li>
        <li><?= $this->Html->link(__('New Menu Item'), ['controller' => 'MenuItems', 'action' => 'add']) ?></li>
    </ul>
</nav>
<div class="menuSubCategories index large-9 medium-8 columns content">
    <h3><?= __('Menu Sub Categories') ?></h3>
    <table cellpadding="0" cellspacing="0">
        <thead>
            <tr>
                <th scope="col"><?= $this->Paginator->sort('id') ?></th>
                <th scope="col"><?= $this->Paginator->sort('menu_category_id') ?></th>
                <th scope="col"><?= $this->Paginator->sort('name') ?></th>
                <th scope="col"><?= $this->Paginator->sort('status') ?></th>
                <th scope="col" class="actions"><?= __('Actions') ?></th>
            </tr>
        </thead>
        <tbody>
            <?php foreach ($menuSubCategories as $menuSubCategory): ?>
            <tr>
                <td><?= $this->Number->format($menuSubCategory->id) ?></td>
                <td><?= $menuSubCategory->has('menu_category') ? $this->Html->link($menuSubCategory->menu_category->name, ['controller' => 'MenuCategories', 'action' => 'view', $menuSubCategory->menu_category->id]) : '' ?></td>
                <td><?= h($menuSubCategory->name) ?></td>
                <td><?= h($menuSubCategory->status) ?></td>
                <td class="actions">
                    <?= $this->Html->link(__('View'), ['action' => 'view', $menuSubCategory->id]) ?>
                    <?= $this->Html->link(__('Edit'), ['action' => 'edit', $menuSubCategory->id]) ?>
                    <?= $this->Form->postLink(__('Delete'), ['action' => 'delete', $menuSubCategory->id], ['confirm' => __('Are you sure you want to delete # {0}?', $menuSubCategory->id)]) ?>
                </td>
            </tr>
            <?php endforeach; ?>
        </tbody>
    </table>
    <div class="paginator">
        <ul class="pagination">
            <?= $this->Paginator->first('<< ' . __('first')) ?>
            <?= $this->Paginator->prev('< ' . __('previous')) ?>
            <?= $this->Paginator->numbers() ?>
            <?= $this->Paginator->next(__('next') . ' >') ?>
            <?= $this->Paginator->last(__('last') . ' >>') ?>
        </ul>
        <p><?= $this->Paginator->counter(['format' => __('Page {{page}} of {{pages}}, showing {{current}} record(s) out of {{count}} total')]) ?></p>
    </div>
</div>
