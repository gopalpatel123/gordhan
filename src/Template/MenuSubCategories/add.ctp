...
<?php $this->set("title", 'Menu-Sub-Categories | '.$coreVariable['company_name']); ?>
...
<div class="portlet box blue-hoki">
			<div class="portlet-title">
				...
				<div class="tools">
					<?php if(!empty($id)){ ?>
						<?php echo $this->Html->link('<i class="fa fa-plus"></i> Add ','/MenuSubCategory/add/',array('escape'=>false,'style'=>'color:black;'));?>
					<?php }?>
				</div>
				<div class="row">	
						...
						<div class="form-group">
							...
							<div class="col-md-8">
								<div class="input-icon right">
									<i class="fa"></i>
									<?php echo $this->Form->input('menu_category_id',['options' =>$menuCategories,'label' => false,'class'=>'form-control select2 selectState','empty'=> 'Select...']);?>
								</div>
							</div>
						</div>
						<div class="form-group">
							<label class="control-label col-md-4">Sub Category Name  <span class="required"> * </span>
							</span>
							</label>
							<div class="col-md-8">
								<div class="input-icon right">
									<i class="fa"></i>
									<input type="text" <?php if(!empty($id)){ echo "value='".$menuSubCategory->name."'"; } ?> name="name" class="form-control" Placeholder="Enter Sub Category Name">
								</div>
							</div>
		...
		<div class="portlet box blue-hoki">
			...
			<div class="portlet-body">
				<table class="table table-str table-hover " cellpadding="0" cellspacing="0" id="main_tbody">
					...
					<tbody>
						<?php $x=0; foreach ($menuSubCategories as $country): 
						//pr($menuSubCategories);die;
						?>
						
						<tr class="main_tr">
							<td><?= ++$x; ?></td> 
								<td><?= h($country->name) ?></td>
							<td><?= h($country->menu_category->name) ?></td>
						
						
							<td class="actions">
								<?php
									if($country->is_deleted==0){
									echo $this->Html->link('Edit ', '/MenuSubCategories/add/'.$country->id, ['class' => 'btn btn-xs blue showLoader']);
								...
								<div id="deletemodal<?php echo $country->id; ?>" class="modal fade" role="dialog">
									<div class="modal-dialog modal-md" >
										<form method="post" action="<?php echo $this->Url->build(array('controller'=>'MenuSubCategories','action'=>'delete',$country->id)) ?>">
							    ...
							    <div id="undeletemodal<?php echo $country->id; ?>" class="modal fade" role="dialog">
									<div class="modal-dialog modal-md" >
										<form method="post" action="<?php echo $this->Url->build(array('controller'=>'MenuSubCategories','action'=>'undelete',$country->id)) ?>">
											<div class="modal-content">
												<div class="modal-header">
													<h4 class="modal-title">
